...
/// <summary>
/// Orchestrates the CompleteWorkOrder process across the WorkOrders, Assets, and Technicians services.
/// 
/// The saga starts when a WorkOrderCompletedEvent is received. It saves basic work order details,
/// sets a 30-second timeout, and publishes a WorkOrderCompletionRequestedEvent so the Assets and
/// Technicians services can finish their parts in parallel.
/// 
/// While in the Processing state, the saga waits for two events:
/// - AssetMaintenanceCompletedEvent
/// - TechnicianAssignmentCompletedEvent
/// 
/// If both events arrive, the saga finishes successfully. If either service failed
/// or the timeout is reached, the saga publishes a WorkOrderCompletingFailedEvent so the
/// WorkOrders service or other services can revert back.
/// </summary>
